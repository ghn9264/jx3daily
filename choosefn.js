        ...
        if (str.includes("花价")) {
            if (str.split(" ").length==2) {
                async function a() {
                    ...
                    let data1 = {'server':'破阵子','flower':str.split(" ")[1],'random':$('input[name=random]').val(),'token':$('input[name=token]').val()}
                    const htmlfinal = await axios.post("http://hua.arkwish.com/",qs.stringify(data1))
                    $ = cheerio.load(htmlfinal.data);
                    let res = $('tbody[class=tb]').html()
                    res = eval('"' + res.replace(/&#x(.{4});/g,"\\u$1") + '"')
                    if (($('.tb').find('td').length>10)) {
                        await bot('send_group_msg', { group_id: data.group_id, message: $('.tb').find('td')[0].children[0].data+","+$('.tb').find('td')[1].children[0].data+","+$('.tb').find('td')[4].children[0].data+"\n"+$('.tb').find('td')[5].children[0].data +","+$('.tb').find('td')[6].children[0].data+","+$('.tb').find('td')[9].children[0].data+"\n"+$('.tb').find('td')[10].children[0].data+","+$('.tb').find('td')[11].children[0].data+","+$('.tb').find('td')[14].children[0].data})
                    }else{
                        await bot('send_group_msg', { group_id: data.group_id, message: $('.tb').find('td')[0].children[0].data+","+$('.tb').find('td')[1].children[0].data+","+$('.tb').find('td')[4].children[0].data+"\n"+$('.tb').find('td')[5].children[0].data +","+$('.tb').find('td')[6].children[0].data+","+$('.tb').find('td')[9].children[0].data})
                    }
                }
